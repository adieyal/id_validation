...
@pytest.fixture
def apartheid_validator() -> SouthAfricaValidator:
...
@pytest.fixture
def invalid_id_numbers(invalid_date_numbers, invalid_citizenship_numbers, invalid_checksum_numbers):
    return invalid_date_numbers + invalid_citizenship_numbers + invalid_checksum_numbers

@pytest.fixture
def invalid_race_id_numbers():
    return ["7106245929885"]
...
@pytest.fixture
def invalid_id_numbers(invalid_date_id_numbers, invalid_citizenship_id_numbers, invalid_checksum_id_numbers):
    return invalid_date_id_numbers + invalid_citizenship_id_numbers + invalid_checksum_id_numbers
    
class TestSouthAfricaValidator:
    def test_clean_idnunber(self, dummy_validator: SouthAfricaValidator):
        assert "7106245929185" == dummy_validator._clean_id_number("7106245 929 185    ")
    ...
    def test_invalid_dob(self, dummy_validator: SouthAfricaValidator, valid_id_numbers, invalid_date_id_numbers):
    ...
    def _test_validate(self, validator: SouthAfricaValidator,
    ...
    def test_validate(self, dummy_validator: SouthAfricaValidator, valid_id_numbers, invalid_date_id_numbers, invalid_checksum_id_numbers):
        for idno in valid_id_numbers:
            assert dummy_validator.validate(idno)
    ...
    def test_generate_checksum(self, dummy_validator: SouthAfricaValidator, valid_id_numbers):
        for idno in valid_id_numbers:
            assert int(idno[-1]) == dummy_validator.generate_checksum(idno[:-1])

    def test_extract_dob(self, dummy_validator: SouthAfricaValidator, valid_id_numbers, invalid_id_numbers):
        for idno in invalid_id_numbers:
    ...
    def test_extract_gender(self, dummy_validator: SouthAfricaValidator, invalid_id_numbers: idnumbers,  male_id_numbers: idnumbers, female_id_numbers: idnumbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
    ...
    def test_extract_checksum(self, dummy_validator: SouthAfricaValidator, valid_id_numbers, invalid_id_numbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
    ...
    def test_extract_data(self, dummy_validator: SouthAfricaValidator, valid_id_numbers, invalid_id_numbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
...
class TestApartheidSouthAfricaValidator:
    def test_race(self, apartheid_validator: ApartheidSouthAfricaValidator):
        for i in range(10):
            idno = f"7106245929{i}85"
            is_valid = apartheid_validator._validate_race(idno)
    ...
    def test_validate(self, apartheid_validator: SouthAfricaValidator, valid_id_numbers, invalid_race_id_numbers):
        for idno in valid_id_numbers:
            assert apartheid_validator.validate(idno)

        for idno in invalid_race_id_numbers:
            assert not apartheid_validator.validate(idno)

    def test_extract_race(self, apartheid_validator: ApartheidSouthAfricaValidator, invalid_id_numbers, valid_id_numbers):
        for idno in invalid_id_numbers:
            with pytest.raises(ValidationError):
                apartheid_validator.extract_race(idno)
    
        for race in list(RACE):
            idno = apartheid_validator.generate_idno(race=race)
            extracted_race = apartheid_validator.extract_race(idno)
            assert extracted_race == race

    def test_extract_data(self, apartheid_validator: ApartheidSouthAfricaValidator, invalid_id_numbers: idnumbers, valid_id_numbers: idnumbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
                apartheid_validator.extract_data(idno)

        for race in list(RACE):
            idno = apartheid_validator.generate_idno(race=race)
            data = apartheid_validator.extract_data(idno)
            assert "race" in data and data["race"] == race
            assert "dob" in data and data["dob"] == apartheid_validator.extract_dob(idno)
            assert "gender" in data and data["gender"] == apartheid_validator.extract_gender(idno)
            assert "checksum" in data and data["checksum"] == apartheid_validator.extract_checksum(idno)

        
            
        
class TestPostApartheidValidator:
    def test_citizenship(self, post_apartheid_validator: SouthAfricaValidator, valid_id_numbers, invalid_citizenship_id_numbers):
    ...
    def test_validate(self, post_apartheid_validator: PostApartheidSouthAfricaValidator, valid_id_numbers: idnumbers, invalid_citizenship_id_numbers: idnumbers):
        for idno in valid_id_numbers:
    ...
    def test_extract_citizenship(self, post_apartheid_validator: PostApartheidSouthAfricaValidator, invalid_id_numbers: idnumbers,  citizen_id_numbers: idnumbers, permanent_resident_id_numbers: idnumbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
    ...
    def test_extract_data(self, post_apartheid_validator: PostApartheidSouthAfricaValidator, invalid_id_numbers: idnumbers, valid_id_numbers: idnumbers):
        for idno in invalid_id_numbers:
            with pytest.raises(SouthAfricaValidationError):
        ...
        for idno in valid_id_numbers:
            ...
            assert "dob" in data and data["dob"] == post_apartheid_validator.extract_dob(idno)
            assert "gender" in data and data["gender"] == post_apartheid_validator.extract_gender(idno)
            assert "checksum" in data and data["checksum"] == post_apartheid_validator.extract_checksum(idno)
            assert "citizenship" in data and data["citizenship"] == post_apartheid_validator.extract_citizenship(idno)
